...
List::~List(){
    ...
    while(li != 0){
        list_item *li_next;
        li_next = li->next; // (*li).next
        delete li;
        li = li_next;
    }     
}

void List::insert_after(list_item *li, list_item *x){// insert x after li
    if(li == 0){
...
void List::delete_at(list_item *li){
    assert(li != 0);
    //(li->prev)<->li<->(li->next) => (li->prev)<->(li->next)
    list_item *li_prev=li->prev, *li_next=li->next;
    if(li == left){
        left = li_next;
    }else li_prev->next = li_next;
    
    if(li == right){
        right = li_prev;
    }else li_next->prev = li_prev;   
